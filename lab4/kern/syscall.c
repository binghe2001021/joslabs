...
//	panic("sys_ipc_recv not implemented");
	if (ROUNDUP(dstva, PGSIZE)!=dstva && ROUNDDOWN(dstva, PGSIZE)!=dstva && dstva<(void*)UTOP)
                return -E_INVAL;
...
sys_sbrk(uint32_t inc)
{
	...
	uint32_t i;
	uint32_t bot = (uint32_t)ROUNDDOWN(curenv->env_sbrk,PGSIZE);
	uint32_t top = (uint32_t)ROUNDUP(curenv->env_sbrk+inc,PGSIZE);
	for (i=bot;i<top;i+=PGSIZE)
	{
		struct Page *newp = page_alloc(ALLOC_ZERO);
		if (newp == NULL)
			panic("sys_sbrk:page_alloc error!");
		page_insert(curenv->env_pgdir,newp,(void*)i,PTE_U | PTE_W);
	}
	curenv->env_sbrk+=inc;
...
int32_t
syscall_enter(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, struct Trapframe* tf, uint32_t a5)
{
	lock_kernel();
//	print_trapframe(tf);
	curenv->env_tf = *tf;
