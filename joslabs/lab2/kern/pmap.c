...
page_init(void)
{
	...
	size_t i;
	extern char end[];
	uint32_t upp = (uint32_t)ROUNDUP((char *)end, PGSIZE) - KERNBASE + PGSIZE + ROUNDUP(npages * sizeof(struct Page), PGSIZE);
...
static int
my_check_continuous(struct Page *pp, int num_page)
{
        struct Page *tmp;
        int i;
